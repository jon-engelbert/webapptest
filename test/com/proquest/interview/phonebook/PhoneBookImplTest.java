...
public class PhoneBookImplTest {
	...
	public void shouldAddFindPerson() {
		...
		Person pFound = phoneBook.findPersonFromCache("John", "Smith");
	...
	public void shouldFindInitialPeople() {
		Person pFound = phoneBook.findPersonFromCache("Chris", "Johnson");
		assertNotNull(pFound);
		assertEquals(2, phoneBook.getPeopleFromCache().size());
	}
	...
	public void ExpectToStringToGenerateListingOfPhonebook() {
		...
		Assert.assertThat(phoneBook.toStringFromCache(), CoreMatchers.containsString("name=Chris Johnson, phoneNumber=(321) 231-7876, address=452 Freeman Drive, Algonac, MI"));
		Assert.assertThat(phoneBook.toStringFromCache(), CoreMatchers.containsString("name=Dave Williams, phoneNumber=(231) 502-1236, address=285 Huron St, Port Austin, MI"));
}
...
//    Just for fun... test prepared statements to see how they work with sql injection attempt
	...
	public void shouldPreventSqlInjectionFind() {
		...
		phoneBook.addPerson(p);
		Person pFound = phoneBook.findPersonFromCache("' or", " '1'='1");
		assertNull(pFound);
	}
...
//  Just for fun... test prepared statements to see how they work with sql injection attempt
	...
	public void shouldPreventSqlInjectionInsert() {
		...
		phoneBook.addPerson(p);
		Person pFound = phoneBook.findPersonFromCache("johnny", "mac");
		assertEquals(p,pFound);
	}
